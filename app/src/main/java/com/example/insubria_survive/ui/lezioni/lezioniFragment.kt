...
class lezioniFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val lezioniViewModel =
            ViewModelProvider(this).get(lezioniViewModel::class.java)

        _binding = FragmentLezioniBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.navLessons
        lezioniViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
